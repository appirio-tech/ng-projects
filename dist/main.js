...
$templateCache.put("views/ng-projects.directive.html","<loader ng-show=\"vm.loading\"></loader><ul class=\"flex wrap\"><li ng-repeat=\"project in vm.projects track by $index\" fitted-width=\"fitted-width\" class=\"elevated-bottom\"><header><div class=\"project-name\"><a ng-if=\"project.status != \'INCOMPLETE\' \" ui-sref=\"timeline({ workId: project.id })\"><h5>{{ project.name || \'unnamed project\' }}</h5></a><a ng-if=\"project.status == \'INCOMPLETE\' \" ui-sref=\"submit-work({ id: project.id })\"><h5>{{ project.name || \'unnamed project\' }}</h5></a><p class=\"type\">{{ project.status || \'No status\' }}</p></div><button class=\"clean message-notification\"><avatar class=\"icon\"></avatar></button></header><main ng-if=\"project.status == \'INCOMPLETE\'\" class=\"incomplete flex column middle center\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-incomplete.svg\" class=\"icon big\"/><h6>Project setup incomplete</h6></div><footer class=\"flex bottom center\"><a ui-sref=\"submit-work({ id: project.id })\" class=\"button action\">continue setup</a></footer></main><main ng-if=\"project.status == \'SUBMITTED\'\" class=\"submitted flex column\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-submitted.svg\" class=\"icon big\"/><h6>Project submitted</h6><p>You are waiting for project approval</p></div><footer class=\"flex column middle\"><p>This should take about 1 day</p></footer></main><main ng-if=\"project.status == \'ASSIGNED\'\" class=\"assigned flex column\"><div class=\"flex column middle center flex-grow\"><h6>Meet your co-pilot</h6><avatar avatar-url=\"{{ project.copilot.avatar }}\" class=\"icon big\"></avatar><p>{{ project.copilot.handle || \'n/a\' }}</p></div><footer class=\"flex bottom center\"><a ui-sref=\"messaging({ id: project.id, threadId: \'threadfor-\' + project.id })\" class=\"button action\">send a message</a></footer></main><main ng-if=\"project.status == \'ESTIMATED\'\" class=\"estimated flex column\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/quote-generated.svg\" class=\"icon big\"/><h6>Quote Generated</h6></div><footer class=\"flex bottom center\"><a ui-sref=\"timeline({ workId: project.id })\" class=\"button action\">view quote</a></footer></main><main ng-if=\"project.status == \'APPROVED\'\" class=\"approved flex column\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-approved.svg\" class=\"icon big\"/><h6>Quote Approved</h6><p>Waiting for project launch</p></div><footer class=\"flex column middle\"><p>You\'ll be notified when the project starts</p></footer></main><main ng-if=\"project.status == \'LAUNCHED\' \" class=\"launched flex column\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-launched.svg\" class=\"icon big\"/><h6>Project launched</h6></div><footer class=\"flex center\"><p>Design phase starts in several days.</p></footer></main><main ng-if=\"project.status == \'COMPLETE\'\" class=\"complete flex column middle center\"><div class=\"flex column middle center flex-grow\"><div class=\"house\"><div class=\"icon big checkmark-white\"></div></div><h6>Project complete!</h6></div><footer class=\"flex column middle\"><a ui-sref=\"timeline({ workId: project.id })\" class=\"button action\">deploy my app</a></footer></main><main ng-if=\"project.status == \'CHOOSE_WINNERS\'\" class=\"flex\"><submission-card phase=\"{{project.activeWorkStep.name}}\" class=\"flex flex-grow column middle center\"></submission-card></main><main ng-if=\"project.status == \'WINNERS_SELECTED\' &amp;&amp; project.activeWorkStep.name == \'Final Designs\' \" class=\"winner flex column middle center\"><div class=\"flex column middle center flex-grow\"><div class=\"house\"><avatar></avatar></div><p class=\"name\">{{project.activeWorkStep.winners[0].handle}}</p><h5>Winner!</h5></div><footer class=\"flex column middle\"><p>{{project.activeWorkStep.nextPhase.name}} phase next</p></footer></main><main ng-if=\"project.status == \'WINNERS_SELECTED\' &amp;&amp; project.activeWorkStep.name == \'Design Concepts\'\" class=\"winners flex column middle center\"><ul class=\"flex-grow flex column\"><li ng-repeat=\"winner in project.activeWorkStep.winners\" class=\"winner flex middle\"><avatar></avatar><p class=\"name flex-grow\">{{ winner.handle }}</p><div class=\"rank\">{{ winner.rank }}</div></li></ul><footer class=\"flex column middle\"><p>{{ project.activeWorkStep.nextPhase.name }} phase next</p></footer></main><main ng-if=\"project.status == \'STEP_OPEN\' \" class=\"flex column middle center\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-submitted.svg\" class=\"icon big\"/><h6 ng-if=\"project.activeWorkStep.name == \'Design Concepts\' ||  project.activeWorkStep.name == \'Final Designs\'\">Awaiting {{project.activeWorkStep.name}}</h6><h6 ng-if=\"project.activeWorkStep.name == \'Final Fixes\'\">Awaiting final deliverables</h6></div><footer class=\"flex column middle\"><p ng-if=\"project.activeWorkStep.name == \'Design Concepts\' ||  project.activeWorkStep.name == \'Final Designs\'\">Receive submissions in several days</p><p ng-if=\"project.activeWorkStep.name == \'Final Fixes\'\">Receive final submissions in several days</p></footer></main></li></ul><div ng-hide=\"vm.loading || vm.projects.length &gt; 0\" class=\"no-projects\"><p>You have not started any projects.</p></div>");
...
$templateCache.put("views/submission-card.directive.html","<div class=\"flex column middle center flex-grow\"><img src=\"/images/icon-download.svg\" class=\"icon big\"/><h6 ng-if=\"vm.phase == \'Design Concepts\'\">submissions received</h6><h6 ng-if=\"vm.phase == \'Final Designs\'\">submissions received</h6><p>{{ vm.end }} left to choose winners</p></div><footer class=\"flex center middle\"><a ui-sref=\"timeline({ workId: project.id })\" class=\"button action\">choose winners</a></footer>");}]);
(function() {
  'use strict';
  var directive;
