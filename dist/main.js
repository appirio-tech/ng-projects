...
angular.module("appirio-tech-ng-projects").run(["$templateCache", function($templateCache) {$templateCache.put("views/ng-projects.directive.html","<ul><li ng-repeat=\"project in vm.projects track by $index\"><ul class=\"header\"><li><p class=\"name\">{{ project.name || \'n/a\' }}</p></li><li><p>{{ vm.typeMap[project.requestType] || \'n/a\' }}</p></li></ul><div class=\"preview\"></div><ul class=\"statuses\"><li class=\"icon-house\"><div ng-if=\"project.status == \'Submitted\'\" class=\"icon checkmark small\"></div><div ng-if=\"project.status == \'Incomplete\'\" class=\"icon warning small\"></div><avatar ng-if=\"project.status != \'Incomplete\' &amp;&amp; project.status != \'Submitted\'\" class=\"icon small\"></avatar></li><li class=\"message\">{{ vm.statusMap[project.status] || vm.statusMap[\'Incomplete\'] }}</li><li class=\"action\"><a ng-if=\"project.status == \'Incomplete\'\" ui-sref=\"submit-work\">Continue</a><span ng-if=\"project.status == \'Submitted\'\"><a ui-sref=\"timeline({ workId: project.id })\">Awaiting estimate</a></span><a ng-if=\"project.status == \'Assigned\'\" ui-sref=\"timeline({ workId: project.id })\">Meet your copilot</a><a ng-if=\"project.status == \'Estimate\'\" ui-sref=\"timeline({ workId: project.id })\">View your quote and begin</a><a ng-if=\"project.status == \'Launched\'\" ui-sref=\"timeline({ workId: project.id })\">View project</a><a ng-if=\"project.status == \'Messaged\'\" ui-sref=\"timeline({ workId: project.id })\">View 1 unread message</a></li></ul></li></ul><a ui-sref=\"view-projects.assigned\" ng-show=\"vm.projects.length &lt; 1\">Manage your copilot projects</a>");}]);
(function() {
  'use strict';
  var directive;
