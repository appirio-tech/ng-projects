...
$templateCache.put("views/submission-card.directive.html","<main class=\"solid flex column middle center\"><img src=\"/images/icon-download.svg\" class=\"icon biggest\"/><label ng-if=\"vm.phase == \'Design Concepts\' || vm.phase == \'Final Designs\'\">{{vm.phase}} submissions received</label></main><footer class=\"flex-grow flex column end middle\"><a ui-sref=\"step({projectId: vm.id, stepId: vm.stepId})\" class=\"button action\">choose winners</a><p>{{ vm.end | timeLapse:vm.end:true }} left to choose winners</p></footer>");}]);
(function() {
  'use strict';
  var directive;
