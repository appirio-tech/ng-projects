...
angular.module("appirio-tech-ng-projects").run(["$templateCache", function($templateCache) {$templateCache.put("views/claimed-projects.directive.html","<loader ng-show=\"vm.loading\"></loader><ul class=\"flex wrap\"><li ng-repeat=\"project in vm.projects track by $index\" fitted-width=\"fitted-width\" class=\"elevated-bottom\"><header><a ui-sref=\"copilot-project-details({ id: project.id })\"><h5>{{ project.name || \'Unnamed Project\' }}</h5></a><p class=\"type\">{{ vm.typeMap[project.projectType] }} - {{project.createdAt | date:\'MM/dd/yyyy\'}}</p></header><main ng-if=\"project.status == \'ASSIGNED\'\" class=\"flex column middle center\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-claimed.svg\" class=\"icon big\"/><h6>Project Claimed</h6></div><footer class=\"flex bottom center\"><a ui-sref=\"copilot-project-details({ id: project.id })\" class=\"action button\">estimate</a></footer></main><main ng-if=\"project.status == \'ESTIMATED\'\" class=\"flex column middle center\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/quote-generated.svg\" class=\"icon big\"/><h6>Projected Estimated</h6></div><footer class=\"flex bottom center\"><p>Client is reviewing estimate</p></footer></main><main ng-if=\"project.status == \'APPROVED\'\" class=\"flex column middle center\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-approved.svg\" class=\"icon big\"/><h6>Project Approved</h6></div><footer class=\"flex bottom center\"><a ui-sref=\"copilot-project-details({ id: project.id })\" class=\"action button\">create challenges</a></footer></main><main ng-if=\"project.status == \'LAUNCHED\'\" class=\"flex column middle center\"><div class=\"flex column middle center flex-grow\"><img src=\"/images/project-launched.svg\" class=\"icon big\"/><h6>Project launched</h6></div><footer class=\"flex bottom center\"><p>Check back later for updates</p></footer></main><main ng-if=\"project.status == \'COMPLETE\'\" class=\"complete flex column middle center\"><img src=\"/images/features.svg\" class=\"icon big\"/><h6>Project complete</h6></main></li></ul><div ng-hide=\"vm.loading || vm.projects.length &gt; 0\" class=\"no-projects\"><p>You have not claimed any projects yet.</p></div>");
...
$templateCache.put("views/ng-projects.directive.html","<loader ng-show=\"vm.loading\"></loader><ul class=\"flex wrap\"><li ng-repeat=\"project in vm.projects track by $index\" fitted-width=\"fitted-width\" class=\"elevated-bottom\"><header><div class=\"project-name\"><a ng-if=\"project.status != \'INCOMPLETE\' \" ui-sref=\"timeline({ workId: project.id })\"><h5>{{ project.name || \'unnamed project\' }}</h5></a><a ng-if=\"project.status == \'INCOMPLETE\' \" ui-sref=\"submit-work({ id: project.id })\"><h5>{{ project.name || \'unnamed project\' }}</h5></a><p class=\"type\">{{ vm.typeMap[project.projectType] }} - {{ project.status }}</p></div><button class=\"clean message-notification\"><avatar class=\"icon\"></avatar><div class=\"notification absolute\">88</div></button></header><card ng-if=\"project.status == \'INCOMPLETE\'\" class=\"flex center middle incomplete\"><main class=\"flex column middle center flex-grow\"><img src=\"/images/project-incomplete.svg\" class=\"icon biggest\"/><div class=\"status\">project setup incomplete</div><a ui-sref=\"submit-work({ id: project.id })\" class=\"button hollow\">continue setup</a></main></card><card ng-if=\"project.status == \'SUBMITTED\'\" class=\"flex column submitted\"><main class=\"solid flex column middle center\"><img src=\"/images/project-submitted.svg\" class=\"icon biggest\"/><div class=\"status\">project submitted</div></main><footer class=\"flex-grow flex column end\"><p>You are waiting for project approval</p><p class=\"secondary\">This should take about 1 day</p></footer></card><card ng-if=\"project.status == \'ASSIGNED\'\" class=\"flex column assigned\"><div class=\"flex column middle center flex-grow\"><h6>Meet your co-pilot</h6><avatar avatar-url=\"{{ project.copilot.avatar }}\" class=\"icon big\"></avatar><p>{{ project.copilot.handle || \'n/a\' }}</p></div><footer class=\"flex-grow flex column end\"><a ui-sref=\"messaging({ id: project.id, threadId: \'threadfor-\' + project.id })\" class=\"button action\">send a message</a></footer></card><card ng-if=\"project.status == \'ESTIMATED\'\"><main class=\"solid flex column middle center\"><img src=\"/images/quote-generated.svg\" class=\"icon biggest\"/><div class=\"status\">quote generated</div></main><footer><a ui-sref=\"timeline({ workId: project.id })\" class=\"button action\">view quote</a></footer></card><card ng-if=\"project.status == \'APPROVED\'\" class=\"flex column approved\"><main class=\"solid flex column middle center\"><img src=\"/images/project-approved.svg\" class=\"icon biggest\"/><div class=\"status\">Quote Approved</div></main><footer><p>Waiting for project launch</p><p class=\"secondary\">You\'ll be notified when the project starts</p></footer></card><card ng-if=\"project.status == \'LAUNCHED\' \" class=\"flex column launched\"><main class=\"solid flex column middle center\"><img src=\"/images/project-launched.svg\" class=\"icon biggest\"/><div class=\"status\">Project launched</div></main><footer class=\"flex-grow flex column end\"><p>Design phase starts in several days.</p></footer></card><card ng-if=\"project.status == \'COMPLETE\'\" class=\"complete flex center middle\"><main class=\"flex column middle center flex-grow\"><img src=\"/images/project-complete.svg\" class=\"icon biggest\"/><div class=\"status\">project complete!</div><a ui-sref=\"timeline({ workId: project.id })\" class=\"button hollow\">deploy my app</a></main></card><card ng-if=\"project.status == \'CHOOSE_WINNERS\'\" class=\"flex column\"><submission-card phase=\"{{project.activeWorkStep.name}}\" class=\"flex flex-grow column\"></submission-card></card><card ng-if=\"project.status == \'WINNERS_SELECTED\' &amp;&amp; project.activeWorkStep.name == \'Final Designs\' \" class=\"flex column winner\"><main class=\"flex column middle center flex-grow\"><div class=\"house\"><avatar class=\"icon biggest\"></avatar></div><p class=\"name\">{{project.activeWorkStep.winners[0].handle}}</p><p class=\"winner-text\">winner!</p></main><footer class=\"flex-grow flex column end\"><p>{{project.activeWorkStep.nextPhase.name}} phase next</p></footer></card><card ng-if=\"project.status == \'WINNERS_SELECTED\' &amp;&amp; project.activeWorkStep.name == \'Design Concepts\'\" class=\"flex column winners\"><main><ul class=\"flex column\"><li ng-repeat=\"winner in project.activeWorkStep.winners\" class=\"winner flex middle\"><avatar class=\"icon big\"></avatar><p class=\"name flex-grow\">{{ winner.handle }}</p><div class=\"rank\"><div class=\"value\">{{ winner.rank }}</div><div class=\"unit\">place</div></div></li></ul></main><footer class=\"flex-grow flex column end\"><p>{{ project.activeWorkStep.nextPhase.name }} phase next</p></footer></card><card ng-if=\"project.status == \'STEP_OPEN\'\" class=\"flex column\"><main class=\"solid flex column middle center\"><img src=\"/images/project-submitted.svg\" class=\"icon biggest\"/><div ng-if=\"project.activeWorkStep.name == \'Design Concepts\' ||  project.activeWorkStep.name == \'Final Designs\'\" class=\"status\">Awaiting {{project.activeWorkStep.name}}</div><div ng-if=\"project.activeWorkStep.name == \'Final Fixes\'\" class=\"status\">Awaiting final deliverables</div></main><footer class=\"flex-grow flex column end\"><p ng-if=\"project.activeWorkStep.name == \'Design Concepts\' ||  project.activeWorkStep.name == \'Final Designs\'\">Receive submissions in several days</p><p ng-if=\"project.activeWorkStep.name == \'Final Fixes\'\">Receive final submissions in several days</p></footer></card></li></ul><div ng-hide=\"vm.loading || vm.projects.length &gt; 0\" class=\"no-projects\"><p>You have not started any projects.</p></div>");
